...
val = digitalRead(sensor);   // read sensor value
  if (val == HIGH) {           // check if the sensor is HIGH
    ...
    if (state == LOW) {
      ...
      // take picture by setting temp value for switch case below
...
if (mode == 1)
{
  if (start_capture == 1)
  {
        Serial.println(F("INSIDE myCAM1"));
    myCAM.flush_fifo();
    myCAM.clear_fifo_flag();
    //Start capture
    myCAM.start_capture();
...
uint8_t read_fifo_burst(ArduCAM myCAM)
{
  Serial.println(F("IN FIFO BURST"));
  ...
  const int ARRAY_SIZE = 28; // anything bigger then 28 causes the send to fail
  ...
  while (length--)
  {
    ...
    if (is_header)
    {
      buffer[buffer_index++] = temp;  // Store data in buffer

      if (buffer_index == ARRAY_SIZE) // Check if buffer is full
      {
     //   Serial.println(F("SENDING PACKET"));
        ...
        while (!sendSuccess && retryCount < 3) {
            sendSuccess = nrf24.send(buffer, buffer_index);
            if (!sendSuccess) {
    ...
    {
      is_header = true;
     // Serial.println(F("ACK IMG START"));
      // Make sure not to lose the first two bytes of the header
      buffer[buffer_index++] = temp_last;
      buffer[buffer_index++] = temp;

    }
    if ((temp == 0xD9) && (temp_last == 0xFF)) // If find the end, break while
    {
      // Flush any remaining data in buffer by sending the last packet
      if (buffer_index > 0)
      {

        Serial.println(F("AT END"));
      ...
      // print out the total packets sent
      Serial.println(F("COUNT AT END"));
